...
def r_precision(targets, predictions, max_n_predictions=500):
    ...
    return float(
        len(set(predictions[:target_count]).intersection(
...
def ndcg(relevant_elements, retrieved_elements, k, *args, **kwargs):
    ...
    # merged...
    idcg = dcg(relevant_elements, relevant_elements,
               min(k, len(relevant_elements)))
    if idcg == 0:
...
MetricsSummary = namedtuple('MetricsSummary', [
    'mean_r_precision', 'mean_ndcg', 'mean_plex_clicks', 'coverage'
])
...
def aggregate_metrics(ground_truth, sub, k, candidates):
    ...
    return MetricsSummary(
        ...
        stats.describe(ndcg).mean, stats.describe(plex_clicks).mean, cov)